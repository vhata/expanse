...
class ExpanseDaemon:
    def __init__(self):
        self.expfile = Path(os.environ.get("HOME", "/")) / Path(".expanserc")
        self.expansions = self.load_expansions()
        self.root = None
        self.listener = None
        self.hotkey = None
        self.setup_gui()
    ...
    def load_expansions(self):
        if not self.expfile.exists():
            return {"expansions": {}}
        try:
            with self.expfile.open() as f:
                return json.load(f)
